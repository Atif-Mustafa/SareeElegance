...
import { CustomizationSelection, ColorOption, BlouseMeasurement } from '../types';
...
export const PDP: React.FC = () => {
  ...
  const { products, formatPrice, addToCart, toggleWishlist, isInWishlist, setIsVideoModalOpen } = useStore();

  const product = products.find((p) => p.slug === slug) || products[0];

  const [selectedImageIndex, setSelectedImageIndex] = useState(0);
  const [selectedColor, setSelectedColor] = useState<ColorOption>(product.colors[0] || { name: 'Standard', hex: '#6B1D2F' });
