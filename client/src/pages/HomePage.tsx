...
import { TrustFeaturesStrip } from '../features/home/components/TrustFeaturesStrip';
import { ColorPaletteEdit } from '../features/home/components/ColorPaletteEdit';
import { OccasionCuration } from '../features/home/components/OccasionCuration';
import { VideoShoppingBanner } from '../features/home/components/VideoShoppingBanner';
import { CelebrityLookbook } from '../features/home/components/CelebrityLookbook';
import { ShopTheLookHotspots } from '../features/home/components/ShopTheLookHotspots';
import { ArtisansSection } from '../features/home/components/ArtisansSection';
import { RealWomenGallery } from '../features/home/components/RealWomenGallery';
import { InstagramGrid } from '../features/home/components/InstagramGrid';
import {
  Sparkles,
  ArrowRight,
  ShieldCheck,
  ...
  React.useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (!ignore) setIsCategoriesLoading(false);
      }
    };
    fetchCategories();
    return () => { ignore = true; };
    ...
    const fetchBestsellers = async () => {
      ...
      try {
        const res = await catalogApi.getProducts({
          ...
          limit: 8,
          // Assuming we want to sort by bestseller if supported, or newest
          sort: 'newest'
        });
        if (!ignore) setBestsellers(res.data.map(mapSummaryToLegacyProduct));
      } catch (err) {
        if (!ignore) {
        ...
        if (!ignore) setIsBestsellersLoading(false);
      }
    };
    fetchBestsellers();
    return () => { ignore = true; };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {isCategoriesLoading ? (
            Array(4).fill(0).map((_, i) => (
              <div key={i} className="h-80 bg-stone-200 animate-pulse rounded-2xl"></div>
            ))
          ) : categories.length === 0 ? (
            <div className="col-span-full py-12 text-center text-stone-500">Categories currently unavailable</div>
          ) : (
            ...
            >
              <img
                src={'https://images.unsplash.com/photo-1610030469983-98e550d615ef?auto=format&fit=crop&q=80'}
                alt={cat.name}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-6">
                ...
                <div className="mt-4 flex items-center text-[#D4AF37] text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity translate-y-4 group-hover:translate-y-0">
